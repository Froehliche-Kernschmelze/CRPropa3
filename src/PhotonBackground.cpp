...
*/
    ...
    // calculate pMax and its norm factor via maximum as peps <= 1
    double cnorm = 0.;
    double pMax = 0.;
    for (int i = 0; i < energy.size(); ++i) {
        double prob = prob_eps(energy[i], onProton, E_in, z_in);
        cnorm += prob;
        if (prob > pMax)
            pMax = prob;
    }
...
void Photon_Field::init(std::string filename) {
    ...
    for (int i = 0; i < energy.size(); ++i) {
        for (int j = 0; j < redshift.size(); ++j) {
            photonDensity.push_back(dn_deps[i][j]);
        }
    }
    infile.close();
}  // end init
...
*/
    ...
    double photonDensity = get_photonDensity(eps, z_in);
    if (photonDensity == 0.) {
        return 0.;
    } else {
...
double Photon_Field::get_photonDensity(double eps, double z_in) const {
/*
